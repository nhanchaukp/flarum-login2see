...
class HideContentInPosts extends FormatContent
{
    public function __invoke($serializer, $model, $attributes)
    {
        if (isset($attributes["contentHtml"])) {
            ...
            if ($replied)
                $newHTML = preg_replace('/<login2see>(.*?)<\/login2see>/is', '<div class="login2see"><div class="login2see_title">' . $this->translator->trans('wiseclock-login2see.forum.hidden_content') . '</div>$1</div>', $newHTML);
            else
                $newHTML = preg_replace('/<login2see>(.*?)<\/login2see>/is', '<div class="login2see"><div class="login2see_alert">' . $this->translator->trans('wiseclock-login2see.forum.login_to_see', array('{login}' => '<a class="login2see_login">' . $this->translator->trans('core.forum.discussion_controls.login_button') . '</a>')) . '</div></div>', $newHTML);
